...
def no_estimators(data, labels):
    X = [0.004576, 24.24, 69.2, 103.53, 231.23, 831.05, 2523.46] # random x values
    Y = [1,1,2,2,3,3,4] # random y values

    datasize = sys.getsizeof(data) + sys.getsizeof(labels)
    interpolate_x = round(datasize/(1024*1024),2)
    print("The size of the dataset is", interpolate_x)
 
    # Finding the interpolation
    y_interp = interp1d(X, Y, fill_value='extrapolate')
    return int(np.round(y_interp(interpolate_x),0))

def no_trees(data):
    X = [500, 522, 540, 540, 569, 583, 625, 690, 699, 736, 748, 768, 797, 841, 846, 958, 990, 1000, 1055, 1080, 1080, 1109, 1372, 
         1458, 1473, 1563, 1593, 1728, 1941, 2000, 2000, 2000, 2000, 2000, 2000, 2109, 2310, 2534, 2600, 3190, 3196, 3751, 3772, 
         4601, 4839, 5404, 5456, 5500, 5620, 6118, 6430, 7797, 9873, 10299, 10885, 10992, 11055, 20000, 34465, 45211, 45312, 48842, 
         67557, 70000, 70000] # random x values
    Y = [335, 370, 253, 219, 298, 339, 441, 280, 119, 470, 494, 358, 849, 222, 250, 170, 248, 330, 346, 262, 214, 340, 92, 275, 612, 
         487, 348, 230, 533, 416, 402, 509, 670, 511, 273, 572, 271, 356, 301, 145, 290, 516, 346, 426, 286, 481, 114, 298, 361, 894, 
         623, 617, 801, 258, 807, 451, 502, 539, 707, 804, 643, 887, 968, 713, 887] # random y values

    n_rows = data.shape[0]
    interpolate_x = n_rows
    print("The n_rows of the dataset is", interpolate_x)
 
    # Finding the interpolation
    y_interp = interp1d(X, Y, fill_value='extrapolate')
    print("The number of trees selected is", int(np.round(y_interp(interpolate_x),0)))
    return int(np.round(y_interp(interpolate_x),0))
...
class BaseCascadeForest(BaseEstimator, metaclass=ABCMeta):
    def __init__(
        ...
        self.n_estimators = no_estimators(data, labels)
        self.n_trees = no_trees(data)
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        print("The Number of estimators selected is '%s'", self.n_estimators)
        return self
